...
#endif
{
	...
	water.viewDir = (_WorldSpaceCameraPos - water.positionWS);
	float3 viewDirNorm = SafeNormalize(water.viewDir);
	...
	float thickness = 1-(saturate(water.shadowMask - (water.foam ) - (1-water.fog) - (water.reflectionMask * _TranslucencyReflectionMask)));
	...
	#if UNDERWATER_ENABLED	
	float3 underwaterColor = ShadeUnderwaterSurface(surfaceData.albedo.rgb, surfaceData.emission.rgb, surfaceData.specular.rgb, scene.color, scene.skyMask, backfaceShadows, inputData.positionWS, inputData.normalWS, water.tangentNormal, viewDirNorm,  _ShallowColor.rgb, _BaseColor.rgb, water.vFace);
